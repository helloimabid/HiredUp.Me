// src/app/jobs/location/[location]/page.js
...
export async function generateStaticParams() {
  return JOB_LOCATIONS.map((loc) => ({ location: loc.slug }));
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: `https://hiredup.me/jobs/location/${slug}` },
    openGraph: {
      title: loc.metaTitle,
      description: loc.metaDescription,
      url: `https://hiredup.me/jobs/location/${slug}`,
      siteName: "HiredUp.me",
    },
...
export default async function LocationPage({ params }) {
  ...
  // Uses enhanced_json header.location as primary signal
  const filteredJobs = filterJobsByLocation(allJobs, loc);
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <nav className="flex text-xs font-medium text-slate-500 dark:text-slate-400 mb-6">
              <ol className="flex items-center gap-2">
                ...
                <li>/</li>
                <li className="text-slate-900 dark:text-white">{loc.label}</li>
              </ol>
            </nav>
            ...
            <div className="lg:col-span-2">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-base font-semibold text-slate-900 dark:text-white">
                  Latest Jobs in {loc.label}
                </h2>
                <Link
                  href={`/jobs?location=${encodeURIComponent(loc.label)}`}
              ...
              {filteredJobs.length === 0 ? (
                <div className="text-center py-16 text-slate-500 dark:text-slate-400">
                  ...
                  ></iconify-icon>
                  <p className="text-sm">
                    No jobs found in {loc.label} right now. Check back soon!
                  </p>
                  <Link
                    href="/jobs"
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24">
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-4">
                  Browse by Category in {loc.label}
                </h3>
                ...
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white mt-8 mb-4">
                  Other Locations
                </h3>
                <div className="space-y-2">
                  {JOB_LOCATIONS.filter((l) => l.slug !== slug).map((l) => (
                    <Link
                      key={l.slug}
                      href={`/jobs/location/${l.slug}`}
                      className="flex items-center gap-2 p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-green-300 dark:hover:border-green-600 hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-all text-sm text-slate-700 dark:text-slate-300"
                    >
                      <iconify-icon
                        icon="solar:map-point-linear"
